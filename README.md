# Typewriter solver

## English
### About
This is a small chrome extension that solves exercises on [Typewriter](https://at4.typewriter.at/) for you.
It's completly opensource and doesn't depend on any libraries, except of Chrome API and JavaScript itself.

### Features
- Randomized speed
- Intentional mi**s**stakes
- Settings you can configure

### How to use
There are actually a few options:
- You can send me your login and password on this email (tolik708main@gmail.com) and I'll do all the needed steps for you.
- You can install extension yourself and do everything on your own. For that you'll find a video at the end.
- You can write on my email and we can meet somewhere in Villa Blanka, I'll show how everything works and so on.

### Important
If it helped you - please share!



## Deutsch
### Über
Es handelt sich um eine kleine Chrome-Erweiterung, die alle Tests auf die Website [Typewriter](https://at4.typewriter.at/) löst.
Es ist opensource und benutzt keine libraries, außer von Chrome API und JavaScript selbst.

### Merkmale
- Zufällige Geschwindigkeit
- Absichtliche F**ä**hler
- Einstellungen die Sie ändern können

### Wie man es einstellt
Es gibt mehrere Optionen:
- Sie können mir Ihre Logindaten per E-Mail (tolik708main@gmail.com) send und ich werde alles Nötige erledigen.
- Sie können die Erweiterung selbst installieren. Dafür habe ich am Ende dieses Dokuments ein Video hinzugefügt.
- Sie können mir eine E-Mail schreiben und wir können uns irgendwo in Villa Blanka treffen. Ich werde Sie alles zeigen.

### Wichtig
Wenn es Ihnen geholfen hat, leiten Sie es weiter!

[Guide.webm](https://github.com/user-attachments/assets/5369fa11-fbcc-4fca-a020-ead2ed0aec7f)
